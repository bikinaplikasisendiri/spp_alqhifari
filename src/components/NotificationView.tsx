...
export default function NotificationView({
  ...
  const searchedOverdueList = useMemo(() => {
    return overdueUnpaidList.filter(item => {
      return item.siswa.nama.toLowerCase().includes(searchTerm.toLowerCase()) ||
             String(item.siswa.nis).includes(searchTerm) ||
  ...
  const handleOpenWhatsAppLink = (siswa: Siswa, bulanName: string, jumlahText: string, total: number) => {
    ...
    if (!phone) {
      toast.error("No. WhatsApp Orang Tua tidak diset! Harap edit biodata siswa dan isi telepon wali terlebih dahulu.");
      return;
    }
  ...
  const handleSendCustomMessage = () => {
    ...
    if (!address || address === "-") {
      toast.error(`Kontak ${type} orang tua belum diset untuk siswa ${siswa.nama}!`);
      return;
    }
    ...
    // Dispatch
    onAddLog(newLog);
    toast.success(`Notifikasi pengingat via ${type} berhasil dikirim dan diarsipkan di log!`);
  ...
  const handleBulkSimulate = () => {
    if (overdueUnpaidList.length === 0) {
      toast.warning("Tidak ada siswa dengan iuran tertunggak atau jatuh tempo!");
      return;
    }
    ...
    setTimeout(() => {
      ...
      toast.success(`Sukses mengirimkan pengingat email otomatis massal ke ${successCount} orang tua/wali siswa! Log tercatat.`);
    }, 1500);
  };
