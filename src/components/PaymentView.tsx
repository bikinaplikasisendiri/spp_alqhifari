...
export default function PaymentView({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedSiswa) {
      toast.warning("Harap pilih siswa terlebih dahulu.");
      return;
    }
    
    if (jumlah <= 0) {
      toast.error("Harap masukkan nominal tagihan yang valid.");
      return;
    }
    ...
    if (isPartialPayment) {
      if (jumlahBayarSekarang <= 0) {
        toast.error("Harap masukkan jumlah pembayaran sebagian yang valid.");
        return;
      }
      if (jumlahBayarSekarang >= jumlah) {
        toast.warning("Jumlah bayar sebagian harus lebih kecil dari total tagihan. Jika ingin melunasi seluruhnya, silakan matikan opsi Bayar Sebagian.");
        return;
      }
    }

    if (metode === 'Cash' && payValue < actualPaid) {
      toast.error(`Uang bayar kurang! Harap bayar minimal ${formatRupiah(actualPaid)}`);
      return;
    }
