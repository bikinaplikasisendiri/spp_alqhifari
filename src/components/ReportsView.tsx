...
export default function ReportsView({ transaksiList, config }: ReportsViewProps) {
  ...
  const exportToXlsx = () => {
    ...
    const data = filteredTrx.map(t => ({
      "No. Jurnal (Kuitansi)": t.id,
      "Siswa NIS": t.siswaNis,
      "Nama Siswa": t.siswaNama,
      "Kelas": t.siswaKelas,
      "Tanggal Transaksi": t.tanggal,
      "Kategori Pembayaran": t.jenisPembayaran,
      "SPP Bulan": t.bulanCovered || "-",
      "Nominal (Rp)": t.jumlah,
      "Metode Bayar": t.metode,
      "Admin Penerima": t.penerima
    }));
    ...
    link.setAttribute("download", `Laporan_Pembayaran_${selectedMonth}_${selectedType}.xlsx`);
